...
The experiment scripts under ``experiments/`` each reproduce one section of the
...
(person columns). The records are loaded as
:class:`entity_resolution.generate_data.Person` so the embedding text and
comparison columns stay identical to the synthetic path.
"""
...
from entity_resolution.entity_pipeline import Blocker, default_comparisons
...
def perturbed_case_tuples(
    ...
    """
    from entity_resolution.person_perturbation import PersonPerturbator, Perturbation
