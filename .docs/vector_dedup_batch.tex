...
\label{eq:gapweight}
\end{equation}
where $\Delta t$ is the age gap between the two records and $T$ the residency timescale
(here 10 years). The identity score always counts at full weight; the address score is
down-weighted exponentially as records age apart, so an old address cannot drag a true
long-gap duplicate below top-$k$ while it still boosts recent pairs. This is a smooth
generalisation of the binary rule, and the address weight is applied at \emph{score-fusion
time} (after retrieval, where the pair-wise gap is known) rather than baked into a single
precomputed vector. In the long gap the gap-weighted view recovers to $1.0$ by $k{=}5$ and
to $0.99$ already at $k{=}1$ (Table~\ref{tab:temporal-recall}), matching the robust
all-identity result while still exploiting the address for recent records. The cost is a
small $k{=}1$ penalty in the short gap ($0.91$ versus the ``full`` view's $1.00$), because
at $k{=}1$ the fused ranking can surface a single address-similar but identity-different
record ahead of the true mate; by $k{\gtrsim}5$ the penalty disappears and recall is $1.0$.
For the working $k$ range (10--20) the gap-weighted view is therefore uniformly the best
choice among the single-vector alternatives.
...
DeepBlocker~\cite{das2020deepblocker}. Learned blocking functions that select candidate
