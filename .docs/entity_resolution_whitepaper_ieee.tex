...
\author{\IEEEauthorblockN{Denis Robert}\IEEEauthorblockA{\textit{Independent Researcher}}}
...
\[
\begin{aligned}
E[TP(\tau)]&=\sum_{j\in D_\tau}p_j,\\
E[FP(\tau)]&=\sum_{j\in D_\tau}(1-p_j),\\
E[FN(\tau)]&=\sum_{j\notin D_\tau}p_j,
\end{aligned}
\]
so the expected F1 reduces to
\[
E[F_1(\tau)]=\frac{2\,S_\tau}{|D_\tau|+N},
\]
where $S_\tau=\sum_{j\in D_\tau}p_j$ is the expected number of true positives and $N=\sum_{j}p_j$ the total expected number of matches.
