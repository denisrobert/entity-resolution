...
The supervised calibration is well behaved and changes the operating point in a measurable way. Recall is essentially unchanged (99.15\% to 99.20\%), while precision falls from 96.83\% to 89.11\% and specificity from 93.50\% to 75.75\%, giving an F1 of 93.88\% versus 97.97\% untrained. In absolute terms the trained variant trades a small reduction in false negatives (34 to 32) for a substantially larger increase in false positives (130 to 485). The results therefore confirm that reported figures are sensitive to how $m/u$ are obtained, and they motivate the labelled-pair and differentiable calibration directions discussed in Section~\ref{sec:further-research}. As a rule, parameters should be trained on labelled operationally representative pairs and evaluated for calibration before reported numbers are treated as production expectations.


\section{Further Research}
\label{sec:further-research}
\subsection{Differentiable tuning of linkage parameters}
The useful back-propagation target is the linkage model after blocking, not the entire retrieval-and-linkage pipeline. FAISS top-$k$ selection is a discrete operation: a record omitted from the candidate set has no comparison feature through which a gradient can flow. The proposed method therefore holds the candidate pairs fixed and uses labelled pairs to tune the comparison model's $m/u$ parameters, field weights, thresholds, missing-value treatment, and, where possible, the comparison functions themselves. Blocking recall must be measured separately and remains an upper bound on end-to-end recall.

For a candidate pair $(q,r)$ and field $f$, first define a differentiable similarity $s_f(q,r) \in [0,1]$. Examples include a character $n$-gram cosine similarity, a soft edit similarity, or a differentiable approximation to Jaro--Winkler. Let $o_f(q,r) \in \{0,1\}$ indicate that both values are observed. For comparison level $j$ with threshold $t_{fj}$ and temperature $T_f>0$, define the soft agreement feature
\[
 c_{f j,\theta}(q,r) = o_f(q,r)\,
 \sigma\!\left(\frac{s_f(q,r)-t_{fj}}{T_f}\right),
 \qquad
 \sigma(x)=\frac{1}{1+\exp(-x)}.
\]
The complete soft comparison vector is
\[
 c_\theta(q,r)=\bigl(c_{fj,\theta}(q,r),\ o_f(q,r),\ 1-o_f(q,r)\bigr)_{f,j}.
\]
The observation indicators permit missingness to have its own learned evidence instead of silently treating a missing value as disagreement. Exact date-of-birth agreement can be included as a fixed binary feature; if a graded date comparison is desired, it can instead use a differentiable distance-based similarity.

To make the connection to Splink explicit, parameterize each soft comparison's match and non-match probabilities as
\[
 m_{fj}=\sigma(a^m_{fj}),\qquad u_{fj}=\sigma(a^u_{fj}),
 \qquad
 w_{fj}=\log\frac{m_{fj}(1-u_{fj})}{u_{fj}(1-m_{fj})}.
\]
Here $m_{fj}$ is the probability of observing agreement level $j$ for a true match, $u_{fj}$ is the corresponding probability for a non-match, and $w_{fj}$ is its log-likelihood-ratio weight. A differentiable linkage score can then be written as
\[
 z_\theta(q,r)=b_\theta+
 \sum_{f,j}w_{fj}c_{fj,\theta}(q,r)+
 \sum_f\left(v_f^\mathrm{obs}o_f(q,r)+v_f^\mathrm{miss}(1-o_f(q,r))\right),
 \qquad
 p_\theta(q,r)=\sigma\!\left(z_\theta(q,r)\right).
\]
The intercept $b_\theta$ represents the prior log-odds of a match. The $v$ terms allow observed and missing fields to contribute separately, while the $w$ terms are soft analogues of Splink's agreement-level log-likelihood ratios. A production decision threshold is applied to $p_\theta$ after training; it is not required to be differentiated during pairwise training.

For labelled pairs $(q_i,r_i,y_i)$, $y_i\in\{0,1\}$, the concrete training objective is regularized binary cross-entropy:
\[
 \mathcal{L}(\theta)=
 -\frac{1}{n}\sum_{i=1}^{n}
 \left[y_i\log p_\theta(q_i,r_i)+
 (1-y_i)\log\left(1-p_\theta(q_i,r_i)\right)\right]
 +\lambda_w\lVert w-w_0\rVert_2^2
 +\lambda_a\lVert a-a_0\rVert_2^2
 +\lambda_T\lVert\log T-\log T_0\rVert_2^2.
\]
The first term trains discrimination. The second keeps learned evidence weights near initial Splink or domain-informed values, the third regularizes the transformed $m/u$ parameters, and the fourth prevents temperatures from collapsing into unstable hard thresholds. The sigmoid parameterizations keep $m$, $u$, and $T$ in valid ranges; additional monotonicity constraints such as $m_{fj}>u_{fj}$ can be enforced by reparameterizing $a^m_{fj}=a^u_{fj}+\operatorname{softplus}(\delta_{fj})$. All terms have well-defined partial derivatives with respect to the trainable parameters.

Comparison algorithms can be tuned only when expressed as a differentiable relaxation. For example, if $s_f^{(1)},\ldots,s_f^{(K)}$ are candidate similarities, use
\[
 s_f(q,r)=\sum_{k=1}^{K}\operatorname{softmax}(\gamma_f)_k\,s_f^{(k)}(q,r),
\]
where $\gamma_f$ learns a mixture of character, token, and edit-based similarities. A discrete algorithm choice cannot receive a useful gradient directly; the relaxed mixture can be optimized and then evaluated or distilled as a discrete production choice.

A practical procedure is therefore:
\begin{enumerate}[leftmargin=*]
  \item build a labelled training set from FAISS candidate pairs, including hard negatives and missingness patterns;
  \item freeze FAISS candidates and compute the soft comparison features for each pair;
  \item initialize $m/u$, field weights, thresholds, temperatures, and optional similarity-mixture parameters from the current Splink configuration;
  \item optimize $\mathcal{L}(\theta)$ with gradient descent, using validation data to select regularization and the final decision threshold;
  \item calibrate the resulting probabilities and compare blocking recall plus final precision, recall, F1, and calibration against the original Splink model; and
  \item distill the trained parameters into Splink comparison levels or retain the differentiable scorer as a separate training-time model, while keeping the production scoring path interpretable and reproducible.
\end{enumerate}

This procedure does not claim end-to-end gradients through FAISS. If embedding parameters are later tuned, they require a separate retrieval objective, such as contrastive loss over positives and hard negatives, and a separate evaluation of top-$k$ blocking recall. The linkage objective should not be used to imply that a missing true candidate can be recovered by changing Splink parameters.
