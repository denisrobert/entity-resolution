...
The compact serialization was the strongest of the three in this run, improving top-20 blocking recall from 99.61\% to 99.76\% and F1 at threshold 0.85 from 99.104\% to 99.175\% (the false-match rate is 0.44\% for default and 0.38\% for compact here). The difference is confirmed at reduced scale by the repeated-seed companion (\texttt{experiment\_section7\_repeated.py}, five seeds at $n{=}1500$; artifact \texttt{results/erwhitepaper/section7\_repeated\_results.json}), which reproduces the compact-over-default ordering with an inter-seed spread ($\approx 0.02$--$0.04$ F1 points) far smaller than the single-seed compact-vs-default gap, under the term-frequency-adjusted scorer. Serialization is thus a small but reproducible retrieval lever on this population. For the default strategy, index build time was 52.4 seconds, p95 blocking latency 8.8\,ms, and estimated mean end-to-end latency 20.0\,ms (batched throughput; the per-query online path is measured separately in Section~\ref{sec:confusion-matrix}). The corresponding values were 44.8 seconds, 8.6\,ms, and 19.7\,ms for identity-first, and 35.4 seconds, 7.5\,ms, and 18.9\,ms for compact.
...
Typo in identity (name or DOB) & 5,000 & 99.7\% & 99.7\% \\
...
A grid search over the decision threshold $\tau$ and the address comparison weight was run on the same 5,000-record / 40,013-query perturbed confusion-matrix population. For each address weight the pipeline was scored once (all candidate pairs emitted at threshold zero) and each candidate decision threshold applied afterwards, giving 30 operating points for a small computational cost. The two findings below concern $\tau$ (where tuning helps, but only marginally) and the address weight (where it does not, on this data set); the address findings are interpreted together with the temporal-decay analysis in Section~\ref{sec:address-decay}.
...
\paragraph{Weakening the address comparison does not help.}
The intuition that address is too volatile to trust as an identity signal was tested by scaling the address agreement probabilities down (``weakening'' the comparison) and re-running the sweep for strengths 0.6--0.9 (against the full-strength 1.0). On the perturbed 5,000-record population the approach did not meaningfully change the outcome: at the working threshold $\tau=0.85$ the address-0.8 variant scored F1 $99.06\%$ (versus $99.104\%$ at full strength), and after threshold tuning the best weakened configuration reached F1 $99.09\%$ --- tied with the full-strength comparison. Downgrading the address evidence therefore costs little on this data set (at most a few false positives at $\tau=0.85$), but it buys nothing either: address agreement here is genuinely informative and does not need to be hedged. The reason, the scope caveats, and how this result sits alongside the temporal-decay evidence are discussed together in Section~\ref{sec:address-decay}.
...
These directions are worth also tempering against the measured F1 budget. Measured on the same query set as the headline confusion matrix (Section~\ref{sec:confusion-matrix}), the default top-20 blocking recall is $99.61\%$ overall (identical $100\%$, close same-entity $97.7\%$), and measured end-to-end recall is a close $98.24\%$: the scorer rejects only 4 of the 39,398 blocked positives, so the linkage stage eats almost none of the retrieved recall and the \emph{blocking stage is now the binding constraint}, not the threshold. The same ceiling holds when the queries are clerical perturbations measured at the blocking stage alone (Section~\ref{sec:blocking-recall-perturbed}): mean top-20 blocking recall under the six perturbation types on the 50k index is $98.63\%$ (rank-1 mean $90.6\%$). A perfect blocker at rank 20 would add at most $0.43$ points of recall ($99.61\%$ to $100\%$); the larger, structural gap is at rank 1 --- $90.6\%$ mean recall@1 across the six perturbations versus the $98.24\%$ the linker achieves end-to-end on its candidate set --- which is where a stronger embedder or multi-view retrieval has visible headroom, particularly for the initial-first-name and close-variation positives. Accordingly, the earlier prioritisation is reinforced: retrieval-side research is currently the higher-impact lever, with the decision threshold a secondary knob as shown by the flat $\tau$ sweep above.
...
The measured experiments also isolate which control levers move F1 across the validation population and which do not. On the perturbed deck, \emph{blocking recall is now the binding constraint}: top-20 blocking recall is 99.74\% for the default serialization of the Section 7 query set (99.61\% on the confusion-matrix population, and 98.63\% mean under the six clerical perturbations at the 50k index) and a perfect blocker at rank 20 would add at most $\approx 0.5$ points of recall, while the larger rank-1 gap (90.6\% mean recall@1 under the six perturbations vs.\ the 98.24\% the linkage stage achieves end-to-end) is the headroom a stronger embedder or multi-view retrieval could recover. Raising $\tau$ from 0.85 to 0.95 buys little (F1 99.104\% to 99.092\%) because recall, not false positives, is scarce. The row serialization strategy shows a small but repeatable advantage: compact raised top-20 blocking recall to 99.75\% and F1 to 99.18\% in the single-seed Section 7 run (99.18\% at $\tau{=}0.85$ versus 99.08\% default), with a five-seed aggregation confirming a small but outside-seed-noise compact F1 edge (+0.057 points, Section 7). Weakening the address comparison (scaling agreement probabilities down by factors 0.6--0.95) neither helped nor materially hurt on the 5,000-record population (best weakened F1 99.09\%, tied with full strength). Supervised calibration from labelled pairs improves F1 at the inherited operating point (98.79\% versus 98.05\% on the 50\,k index), and EM also improves on the duplicate-bearing 100\,k benchmark (97.85\% versus 95.63\% untrained). Because a fixed match prior keeps the operating point coherent across variants, calibration is stable: supervised and EM-trained $m/u$ match or beat the untrained defaults (Section~\ref{sec:mu-calibration}). The remaining, controllable sensitivity is the choice of the match prior, which should be estimated or verified rather than left to an unsupervised fit. Operationally representative labels and joint threshold/prior calibration therefore remain necessary to lock in the supervised gain at deployment, and the fast online matcher is the vehicle through which any calibrated weights are served.
