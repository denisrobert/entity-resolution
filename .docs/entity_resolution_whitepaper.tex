...
That a comparison's evidence depends on the length of time between two records is a documented phenomenon in the record-linkage literature rather than a novelty of the present pipeline. \textsc{Li} et al.\ introduced \emph{time-decay} to the temporal record-linkage problem, applying a decay to capture the effect of elapsed time on value evolution~\cite{lidong2011}. \textsc{Christen} and \textsc{Gayler} later made the temporal likelihoods adaptive on dynamic databases~\cite{christengayler2013}, and Hu et al.\ estimated the gap-dependent agreement directly with regression classification in temporal record linkage~\cite{hu2017regression}. \textsc{Chiang}, \textsc{Doan}, and \textsc{Naughton} developed the related idea of modeling entity evolution for temporal record matching~\cite{chiang2014}. Most directly, \textsc{Shim}'s recent TimeLink proposal replaces static $m_k$ with a time-dependent $m_k(\Delta t)$ derived from a semi-Markov generative model of field evolution, keeping $u_k$ time-independent, and reports that static Fellegi--Sunter collapses on decade-long gaps~\cite{shim2026}.
...
\]
which feeds the same Fellegi--Sunter log-odds as a discrete level weight. The two densities must be estimated from data, by fitting a normal-mixture model to the score distribution, by calibration against labelled pairs, or by EM~\cite{christen2012}. Winkler~\cite{winkler2006} reviews how comparators' scores are mapped to weights.

The modern, directly relevant line applies the same idea to learned matchers: the end-to-end surveys of Christophides et al.~\cite{christophides2021} position learned matchers and Fellegi--Sunter linkage as complementary stages of one entity-resolution pipeline, and Sadinle and Fienberg~\cite{sadinle2013} generalise the Fellegi--Sunter construction to the multiple-record setting, fitting match patterns from the comparison data by mixture maximum likelihood. Together these yield the practical recipe for this paper's setting: treat the matcher score as the comparison, estimate $f(x\mid M)$ and $f(x\mid U)$ (by mixture modelling~\cite{christen2012}, by calibration against labelled pairs, or by EM), form $w(x)=\log_2[f(x\mid M)/f(x\mid U)]$, and add it to the FS log-odds exactly like a level weight, with the match prior $\lambda$ unchanged.
...
\]
where $\sum_j p_j$ is the total expected number of matches in the candidate-pair set $S$. One then selects $\tau^*=\operatorname{arg\,max}_\tau E[F_1(\tau)]$ over a dense grid. This is a direct use of the calibrated posterior probabilities produced by the mixture/EM fitting of the Fellegi--Sunter model~\cite{christen2012}; it approximates the cost-based rule when the two error types are weighted equally.

\paragraph{Gaussian-mixture valley detection.} The Fellegi--Sunter weights $w_j=\log_2 R(\gamma_j)$ are typically bimodal: a large peak of low-weight non-matches and a smaller high-weight match peak. Fitting a two-component Gaussian mixture~\cite{christen2012} to the weights yields densities $f_U(w)$ and $f_M(w)$ for the non-match and match components; the threshold is placed at the intersection
\[
p\, f_M(w^*)=(1-p)\, f_U(w^*),
\]
