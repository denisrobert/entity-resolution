...
\begin{abstract}
This paper presents and evaluates a two-stage entity resolution pipeline for incomplete Canadian person records. A synthetic reference population is represented with normalized \texttt{all-MiniLM-L6-v2} embeddings and persisted in a FAISS \texttt{IndexFlatIP} store. Each query is first blocked to its top 20 vector neighbors, after which Splink performs interpretable probabilistic linkage using Jaro--Winkler comparisons for names and address, a dedicated email comparison, and a date-of-birth comparison that accounts for date structure. The approach is similar in overall architecture to the FAISS-plus-linkage workflow described by Strojny and Ber\k{e}sewicz~\cite{strojny2026}, although the present pipeline was independently derived and uses a different implementation and evaluation design. The implementation supports persisted-index reload without re-embedding the reference population and independently models 30\% missingness for address and email. In a 15,000-query experiment comprising identical, close-variation, and unrelated records, the pipeline achieved 99.27\% accuracy, 99.51\% precision, 99.40\% recall, 99.02\% specificity, and a 99.45\% F1 score, with an amortized batched processing time (batched FAISS blocking plus one batched Splink inference) of 7.48\,ms per query (amortized batched figures are environment- and run-dependent; the artifact records the run-specific timing, and the per-query online resolver path, which constructs a Splink linker per request, is slower and is discussed in Section~\ref{sec:confusion-matrix}. The pipeline scores with Splink's untrained default $m/u$ and a fixed prior of $10^{-4}$, so the reported thresholds are operating-score cut-offs rather than calibrated posterior probabilities (Section~\ref{sec:prob-linkage}). The pipeline is designed for fast online deduplication in which the operator knows the desired cost profile in advance --- the relative cost of a false positive versus a false negative --- so that the decision threshold can be tuned on a calibrated validation set, which is the setting in which a cost-optimal threshold is well defined. The paper also discusses memory scaling, operational limits, calibration of Splink parameters, and alternatives to linear-scan FAISS indexing.
\end{abstract}
...
The compact serialization was the strongest of the three in this run, improving top-20 blocking recall from 99.88\% to 99.95\% and reducing the false-match rate at threshold 0.85 from 0.98\% to 0.94\%. The difference is descriptive rather than statistically conclusive, and the repeated-seed companion does not confirm a stable advantage at reduced scale: across five seeds at $n{=}2000$, default and compact both average $99.96\%$ R@20, the mean F1 difference is only $0.005$ points, and the per-strategy F1 standard deviation is $\approx 0.03$ points (mean $99.88 \pm 0.03\%$; artifact \texttt{results/erwhitepaper/section7\_repeated\_results.json}, produced by \texttt{experiment\_section7\_repeated.py}). Serialization should therefore be retested on labelled operational data before being treated as a lever, as the effect is within the seed-to-seed noise at this scale. For the default strategy, index build time was 35.7 seconds, p95 blocking latency was 25.5\,ms, and estimated mean end-to-end latency was 20.2\,ms (batched throughput; the per-query online path is slower, Section~\ref{sec:confusion-matrix}). The corresponding values for identity-first were 39.4 seconds, 24.4\,ms, and 20.0\,ms; compact was 30.8 seconds, 20.8\,ms, and 17.1\,ms.
...
\midrule
Actual same entity & 9,940 (TP) & 60 (FN) \\
Actual different entity & 49 (FP) & 4,951 (TN) \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:confusion-matrix} summarises the measured confusion matrix. All 5,000 identical queries were correctly matched. Among the 5,000 close same-entity queries, 4,940 were matched and 60 were missed. Among the 5,000 unrelated queries, 49 were incorrectly accepted and 4,951 were rejected. The resulting accuracy was 99.27\%, precision 99.51\%, recall 99.40\%, specificity 99.02\%, and F1 99.45\%. These are measured at the \emph{amortized batched} operating point: batched FAISS blocking plus one batched Splink inference over the 15{,}000 queries, which in the validation environment was 7.48\,ms per query (an isolated earlier run; the artifact's timing field records the run-specific value, and timing varies with machine state). The \emph{per-query online path} is materially slower: the production resolver constructs a fresh Splink linker and runs a separate inference per request, measured here at roughly 0.9\,s/query (mean; artifact \texttt{results/erwhitepaper/online\_resolver\_latency.json}), of which $\approx 0.24$\,s is per-query Linker construction. The batched figure is therefore a throughput bound, not an online per-request latency, and online deployments should time the resolver path directly rather than quote it. These measurements are a baseline rather than a general performance guarantee; hardware, model caching, and Splink configuration affect both quality and latency.

\paragraph{Blocking recall on the same query set.} The confusion-matrix run also records whether each positive query's true reference was among the top-$k$ FAISS neighbours. On this exact 15,000-query set the default top-20 blocking recall is $99.46\%$ overall (identical queries $100\%$, close same-entity queries $98.92\%$), and Splink rejected only 6 of the 9{,}946 blocked positives. Blocking recall and final linkage recall are therefore measured here from the same artifact and query set, so the two can be compared stage by stage without mixing experiments.
...
The confusion-matrix protocol is rerun with a fixed seed (42), so the same queried population and candidate set are regenerated deterministically. The reported counts are nevertheless subject to small run-to-run variation: Splink's inference runs on DuckDB with parallel workers, and repeated runs of \texttt{experiment\_confusion\_matrix.py} under the same settings returned matrices that differed by one or two counts in ten thousand (e.g., the FN count shifted from 59 to 60 between the originally published artifact and the regenerated one in this revision). Fixed-seed repeatability therefore establishes that the experiment is reproducible in content, not that the counts are exact; a 95\% binomial interval on 60 false negatives out of 10,000 positives spans roughly $\pm 1.5$ counts. Tables should always be read against the persisted artifact named in the caption rather than remembered as exact round numbers, and the conclusions --- which rest on metric differences of whole percentage points, not on single-digit counts --- are insensitive to this variation.
...
\midrule
$10^{-4}$ & 98.7 & 98.7 & 100.0 & 90.9 & 94.9 & 95.5\\
$10^{-3}$ & 96.8 & 96.8 & 96.8 & 80.7 & 80.7 & 82.9\\
$10^{-2}$ & 95.5 & 95.5 & 96.8 & 80.2 & 80.2 & 80.7\\
\bottomrule
\end{tabular}
\end{table}
...
Variant & Accuracy & Precision & Recall & Specificity & F1 \\
\midrule
Untrained (default $m/u$, prior 0.0001) & 94.03\% & 90.70\% & 98.13\% & 89.93\% & 94.27\% \\
Supervised (labelled pairs) & 83.07\% & 75.33\% & 98.33\% & 67.80\% & 85.31\% \\
Unsupervised (EM, prior 0.0071) & 62.13\% & 57.04\% & 98.33\% & 25.93\% & 72.20\% \\
\bottomrule
\end{tabular}
\end{table}

In confusion-matrix terms the untrained variant produced TP \(=1{,}472\), FN \(=28\), FP \(=151\), TN \(=1{,}349\); supervised TP \(=1{,}475\), FN \(=25\), FP \(=483\), TN \(=1{,}017\); and EM TP \(=1{,}475\), FN \(=25\), FP \(=1{,}111\), TN \(=389\). Recall is comparable across all three (98.1--98.3\%), but the trained schemes increasingly sacrifice precision and specificity for that recall: supervised precision falls to 75.3\% and specificity to 67.8\%; EM falls to 57.0\% precision and a very low 25.9\% specificity. The EM run also estimates a much larger match prior (0.0071 versus the fixed 0.0001), which, together with EM-fitted $m/u$, drives the gross over-matching. The untrained default thus yields the best F1 (94.27\%) on this duplicate-bearing population under an entity-disjoint evaluation, reinforcing that the choice of $m/u$ strongly affects the measured operating point and that reported figures must be tied to the exact parameter configuration used.
...
\midrule
0.85 & 99.27\% & 99.51\% & 99.40\% & 99.02\% & 99.45\% \\
0.87 & 99.25\% & 99.52\% & 99.36\% & 99.04\% & 99.44\% \\
0.90 & 99.23\% & 99.52\% & 99.33\% & 99.04\% & 99.42\% \\
0.92 & 99.23\% & 99.52\% & 99.33\% & 99.04\% & 99.42\% \\
0.95 & 99.51\% & 99.94\% & 99.33\% & 99.88\% & 99.63\% \\
\bottomrule
\end{tabular}
\end{table}

At $\tau=0.95$ the confusion matrix is TP \(=9{,}933\), FN \(=67\), FP \(=6\), TN \(=4{,}994\): false positives fall from 49 to 6 (precision 99.51\% to 99.94\%, specificity 99.02\% to 99.88\%) while recall falls only from 99.40\% to 99.33\% (F1 99.45\% to 99.63\%). The improvement is modest but consistent with the analysis above: in a regime where recall has headroom, the decision threshold is an effective precision lever and should be tuned on a validation set rather than fixed at 0.85.

\paragraph{Weakening the address comparison did not help.}
\paragraph{Weakening the address comparison did not help.}
The intuition that address is too volatile to trust as an identity signal was tested by scaling the address agreement probabilities down (``weakening'' the comparison) and re-running the sweep for strengths 0.6--0.9 (against the full-strength 1.0). On this data set the approach did not improve performance: at the working threshold $\tau=0.85$ the address-0.8 variant scored F1 $98.09\%$, and after threshold tuning every weakened configuration topped out near $98.5\%$ F1, well below the $99.63\%$ reached by the full-strength address comparison at $\tau=0.95$. The reason is that the address field here is genuinely informative: 30\% of addresses are missing independently, and the addresses that do exist are long and highly discriminative, so address agreement is valuable evidence for correctly rescuing the close same-entity queries. Downgrading that evidence cost more true positives than it suppressed false ones.
...
These directions are nevertheless worth tempering against the measured F1 budget. Measured on the same query set as the headline confusion matrix (Section~\ref{sec:confusion-matrix}), the default top-20 blocking recall is $99.46\%$ (identical queries $100\%$, close same-entity queries $98.92\%$), and measured end-to-end recall is $99.40\%$: a perfect blocker could therefore add at most $0.60$ points of recall, and even then the F1 could rise only from $99.45\%$ toward at most $99.73\%$ --- a gain of less than three tenths of a point, computed from the same artifact's blocked-but-rejected counts rather than from a different experiment. In other words, on this population alternative embeddings and multi-view retrieval are unlikely to move F1 materially while top-$k$ blocking recall is already near-saturated. Their value would instead emerge in regimes where blocking recall is the binding constraint: much larger and noisier populations, or retrieval configurations that no longer hold near-perfect recall at a small $k$. Absent such a regime, the higher-impact research target remains the Splink linkage stage as tuned through the decision threshold and $m/u$ calibration experiments above.
...
The measured experiments also isolate which control levers move the F1 across the validation population and which do not. Three levers had a clear effect. The decision threshold $\tau$ is the cheapest and most effective precision lever: because recall already sat near its ceiling, raising $\tau$ from 0.85 to 0.95 on the confusion-matrix population lifted F1 from 99.45\% to 99.63\% (precision 99.51\% to 99.94\%, specificity 99.02\% to 99.88\%) at a small recall cost. The row serialization strategy showed no repeatable advantage: the compact representation raised top-20 blocking recall to 99.95\% and F1 to 99.73\% in the single-seed Section 7 run, but a five-seed aggregation found the effect within seed-to-seed noise (mean F1 difference $0.005$ points, $\pm 0.03$) at reduced scale. It should therefore be validated on the deployment population before being relied on as a lever. So is the blocking size $k$, which sets an upper bound on end-to-end recall; the default $k=20$ already achieves 99.88\% blocking recall in the Section 7 run, so increasing $k$ buys only a marginal recall gain while adding retrieval cost. Two levers did not help on this data set. Weakening the address comparison (scaling its agreement probabilities down by factors 0.6--0.95) never improved F1, topping out near 98.5\% rather than the 99.63\% reached at full address strength, because address agreement is genuinely informative for the close same-entity queries here. Likewise, retraining $m/u$ on the synthetic population --- whether supervised or by expectation maximisation --- over-matched and lowered F1 relative to the untrained default at the inherited threshold and prior. In these experiments, supervised calibration did not improve F1 at the inherited operating point; operationally representative labels and joint threshold/prior calibration remain necessary to determine whether it improves deployment performance.
