...
\begin{table}[h]
\centering
\caption{Blocking-recall ablations at $n=500$ per condition (artifact \texttt{results/erwhitepaper/section7\_results.json}).}
...
The confusion-matrix protocol is rerun with a fixed seed (42) and, at this scale, its results are reproducible: repeated runs of \texttt{experiment\_confusion\_matrix.py} under the same settings returned the same confusion matrix. We note, however, that Splink's inference runs on DuckDB with parallel workers, so at much larger scales or under a different backend the exact counts can shift by a few in ten thousand; tables should therefore always be read against the persisted artifact named in the caption rather than remembered as exact round numbers.
...
\begin{table}[h]
\centering
\caption{Trained versus untrained linkage parameters over 50,000 reference records and 6,000 queries ($\tau=0.85$, $k=20$, prior $0.0001$; artifact \texttt{results/erwhitepaper/mu\_calibration\_results.json}).}
...
\begin{table}[h]
\centering
\caption{Large duplicate-bearing benchmark: 100{,}000 base records plus 3{,}000 duplicates (103{,}000-record index) and 6{,}000 queries ($\tau=0.85$, $k=20$; artifact \texttt{results/erwhitepaper/training\_results.json}).}
...
\]
which feeds the same Fellegi--Sunter log-odds as a discrete level weight. The two densities must be estimated from data. The classic treatments are Belin and Rubin~\cite{belin1995}, who calibrate match and non-match rates directly from a continuous weight via a normal-mixture model of the score density, and Copas and Hilton~\cite{copashilton1990}, who model the distribution of a continuous log-likelihood-ratio score under match and non-match. Winkler~\cite{winkler2006} reviews how comparators' scores are mapped to weights.

The modern, directly relevant line applies the same idea to learned matchers: the end-to-end surveys of Christophides et al.~\cite{christophides2021} position learned matchers and Fellegi--Sunter linkage as complementary stages of one entity-resolution pipeline, and Sadinle and Fienberg~\cite{sadinle2013} generalise the Fellegi--Sunter construction to the multiple-record setting, fitting match patterns from the comparison data by mixture maximum likelihood. Together with the continuous-score treatments of Belin--Rubin and Copas--Hilton above, this yields the practical recipe for this paper's setting: treat the matcher score as the comparison, estimate $f(x\mid M)$ and $f(x\mid U)$ (by mixture modelling as in Belin--Rubin, by calibration against labelled pairs, or by EM), form $w(x)=\log_2[f(x\mid M)/f(x\mid U)]$, and add it to the FS log-odds exactly like a level weight, with the match prior $\lambda$ unchanged.
...
\newblock \url{https://doi.org/10.1007/978-3-540-24775-3_75}.
\newblock FEBRL data sets: \url{https://www.kaggle.com/datasets/adrianevensen/restaurants-entity-resolution} and \url{https://sourceforge.net/projects/febrl/}.
...
\newblock \url{https://doi.org/10.1080/01621459.1995.10476563}.
...
\newblock \url{https://doi.org/10.2307/2982975}.
...
\newblock \url{https://doi.org/10.1002/0471721182}.
